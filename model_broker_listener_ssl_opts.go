...
type BrokerListenerSslOpts struct {
	...
	// Path to a file containing PEM-encoded Diffie-Hellman parameters<br/>to be used by the server if a cipher suite using Diffie-Hellman<br/>key exchange is negotiated. If not specified, default parameters<br/>are used.<br/><br/>NOTE: The <code>dhfile</code> option is not supported by TLS 1.3.
	Dhfile string `json:"dhfile,omitempty"`
	// Used together with {verify, verify_peer} by an TLS/DTLS server.<br/>If set to true, the server fails if the client does not have a<br/>certificate to send, that is, sends an empty certificate.<br/>If set to false, it fails only if the client sends an invalid<br/>certificate (an empty certificate is considered valid).
	FailIfNoPeerCert bool `json:"fail_if_no_peer_cert,omitempty"`
	// An important security setting, it forces the cipher to be set based<br/> on the server-specified order instead of the client-specified order,<br/> hence enforcing the (usually more properly configured) security<br/> ordering of the server administrator.
	HonorCipherOrder bool `json:"honor_cipher_order,omitempty"`
	// In protocols that support client-initiated renegotiation,<br/>the cost of resources of such an operation is higher for the server than the client.<br/>This can act as a vector for denial of service attacks.<br/>The SSL application already takes measures to counter-act such attempts,<br/>but client-initiated renegotiation can be strictly disabled by setting this option to false.<br/>The default value is true. Note that disabling renegotiation can result in<br/>long-lived connections becoming unusable due to limits on<br/>the number of messages the underlying cipher suite can encipher.</br><br/>Has no effect when TLS version is configured (or negotiated) to 1.3
	...
	GcAfterHandshake bool `json:"gc_after_handshake,omitempty"`
	Ocsp *BrokerOcsp `json:"ocsp,omitempty"`
	// Whether to enable CRL verification for this listener.
	EnableCrlCheck bool `json:"enable_crl_check,omitempty"`
}
