...
type EmqxGatewayApiLwm2m struct {
	// Gateway Name
	...
	LifetimeMin string `json:"lifetime_min,omitempty"`
	// Maximum value of lifetime allowed to be set by the LwM2M client.
	LifetimeMax string `json:"lifetime_max,omitempty"`
	// The value of the time window during which the network link is considered valid by the LwM2M Gateway in QMode mode.<br/>For example, after receiving an update message from a client, any messages within this time window are sent directly to the LwM2M client, and all messages beyond this time window are temporarily stored in memory.
	QmodeTimeWindow string `json:"qmode_time_window,omitempty"`
	// Automatically observe the object list of REGISTER packet.
	AutoObserve bool `json:"auto_observe,omitempty"`
	// Policy for publishing UPDATE event message.<br/><br/>  - always: send update events as long as the UPDATE request is received.<br/><br/>  - contains_object_list: send update events only if the UPDATE request carries any Object List
	UpdateMsgPublishCondition string `json:"update_msg_publish_condition,omitempty"`
	Translators *EmqxLwm2mSchemaLwm2mTranslators `json:"translators"`
	...
	Listeners []OneOfemqxGatewayApiLwm2mListenersItems `json:"listeners,omitempty"`
}
