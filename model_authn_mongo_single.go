/*
 * EMQX API
 *
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * API version: 5.1.3
 * Generated by: Swagger Codegen (https://github.com/swagger-api/swagger-codegen.git)
 */
package swagger

type AuthnMongoSingle struct {
	// Authentication mechanism.
	Mechanism string `json:"mechanism"`
	// Backend type.
	Backend string `json:"backend"`
	// Collection used to store authentication data.
	Collection string `json:"collection"`
	// Conditional expression that defines the filter condition in the query.<br/>Filter supports the following placeholders:<br/>- <code>${username}</code>: Will be replaced at runtime with <code>Username</code> used by the client when connecting<br/>- <code>${clientid}</code>: Will be replaced at runtime with <code>Client ID</code> used by the client when connecting
	Filter *interface{} `json:"filter,omitempty"`
	// Document field that contains password hash.
	PasswordHashField string `json:"password_hash_field,omitempty"`
	// Document field that contains the password salt.
	SaltField string `json:"salt_field,omitempty"`
	// Document field that defines if the user has superuser privileges.
	IsSuperuserField string `json:"is_superuser_field,omitempty"`
	// Options for password hash verification.
	PasswordHashAlgorithm *OneOfauthnMongoSinglePasswordHashAlgorithm `json:"password_hash_algorithm,omitempty"`
	// Set to <code>true</code> or <code>false</code> to disable this auth provider.
	Enable bool `json:"enable,omitempty"`
	// Standalone instance. Must be set to 'single' when MongoDB server is running in standalone mode.
	MongoType string `json:"mongo_type,omitempty"`
	// The IPv4 or IPv6 address or the hostname to connect to.<br/><br/>A host entry has the following form: `Host[:Port]`.<br/><br/>The MongoDB default port 27017 is used if `[:Port]` is not specified.
	Server string `json:"server"`
	// Write mode.
	WMode string `json:"w_mode,omitempty"`
	// Use DNS SRV record.
	SrvRecord bool `json:"srv_record,omitempty"`
	// Size of the connection pool towards the bridge target service.
	PoolSize int32 `json:"pool_size,omitempty"`
	// The username associated with the bridge in the external database used for authentication or identification purposes.
	Username string `json:"username,omitempty"`
	// The password associated with the bridge, used for authentication with the external database.
	Password string `json:"password,omitempty"`
	// Database name associated with the user's credentials.
	AuthSource string `json:"auth_source,omitempty"`
	// Database name.
	Database string `json:"database"`
	Topology *EmqxMongodbTopology `json:"topology,omitempty"`
	Ssl *BrokerSslClientOpts `json:"ssl,omitempty"`
}
